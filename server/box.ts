...
// into every new name so another Sandbase bot installation using the same Box
 ...
 * carrying Sandbase bot's exact deterministic name shape leave this boundary;
...
export async function listManagedBoxes(
  ...
  if (options.adoptLegacy !== false) {
    ...
    try {
      ...
      return invalidInventory("Sandbase bot could not safely read its cloud computer recovery records");
    }
    for (const recovery of recoveries) {
      ...
      if (matchingRows.length === 1) {
        ...
        if (listedName !== namedOwner.currentName && listedName !== namedOwner.legacyName) {
          return invalidInventory("A remembered cloud computer no longer has its Sandbase bot owner name — repair it in ascii.dev before continuing");
        }
        continue;
      }
    ...
    try {
      ...
      return invalidInventory("Sandbase bot could not safely read its cloud computer deletion records");
    }
  ...
  for (const candidate of candidates) {
    ...
    if (legacyOwner && owner && options.adoptLegacy !== false) {
      try {
        ...
        return invalidInventory("Sandbase bot could not safely remember this legacy cloud computer's owner — repair it in ascii.dev before continuing");
      }
    }
...
/** The keys this Sandbase bot already holds, as the environment its bots'
...
async function requestBoxCreate(cfg: AppConfig, botId: string, ttlSeconds: number, env: Record<string, string>): Promise<BoxCreateResult> {
  ...
  // guest ever has are the ones this Sandbase bot forwards (`env`), which its
...
// Sandbase bot has over any frame off this box: the turn runs on the box's
