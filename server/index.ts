// Sandbase bot server — the harness host. Clients hold no transports
...
const browserCleanup: BrowserCleanupCoordinator = new BrowserCleanupCoordinator({
  ...
  send: (request) => {
    // Cleanup may only run the engine Sandbase bot itself configured or
    ...
    const work = status.kind === "ready" && sessions.length
      ? Promise.all(sessions.map(async (session) => {
          ...
          if (!ok) console.warn(`browser cleanup: could not clear saved state for session ${session}; restart Sandbase bot to retry this profile's cleanup. Do not use state clear --all: it erases other profiles too.`);
          return ok;
        }))
...
function previewSystemPrompt(bot: BotRecord) {
  ...
  const persona = [
    `You are ${bot.name}, a personal bot in Sandbase bot.`,
...
async function mountHostComputer(owner: TurnOwner, botId: string, providerSupportsLocal: boolean) {
  ...
  if (!cua) throw new Error("CUA Driver is not ready for this computer — check permissions and restart Sandbase bot");
...
bus.subscribe((event: RuntimeEvent) => {
  ...
  switch (event.type) {
    ...
    case "request.opened": {
      ...
      // Sandbase bot decides nothing about the action itself. Only Full access
...
store.reconcileInterruptedGroupGoals((runId, threadId) => {
  ...
  const detail = run.output ?? run.error ?? (
    status === "completed"
      ? "The scheduled team goal completed before Sandbase bot restarted."
      : status === "stopped"
        ? "The scheduled team goal was stopped."
        : "Sandbase bot restarted before this scheduled team goal finished."
  );
...
async function cloudRoutineReadiness(): Promise<{ ready: boolean; reason?: string }> {
  ...
  if (!instance) {
    return { ready: false, reason: "The Cloud VM runner is unavailable. Restart Sandbase bot and try again." };
  }
  try {
      ...
      try {
        ...
        if (localVmMode(cfg) === "per-bot") {
          ...
          if (!vm.daemonUp && existsSync(target.workspaceDir)) {
            return deletionResponse( 409, {
              error: "start the container runtime so Sandbase bot can remove this bot's Local VM while deleting it",
            });
          }
          if (vm.container !== "missing" && !vm.managed) {
            return deletionResponse(409, {
              error: `The container named ${vm.container_name} was not created by Sandbase bot. Remove it manually before deleting this bot`,
            });
          }
          localVmCleanup = {
            target,
...
async function runGroupMemberTurn(
  ...
  const outsideRoom = integrations.agents
    ...
    : [];
  const system = [
    `You are ${bot.name}, a bot in the room "${readyGroup.name}" in Sandbase bot.`,
    bot.title && `Role: ${bot.title}.`,
    bot.description && `About: ${bot.description}`,
    ...
    integrations.agents && (!orchestration || orchestration.roomHandoffId) && "For actual Sandbase bot teamwork, discover IDs with list_room_targets and use coordinate_bots for advice or work in this or another room. Do not substitute native coding helpers for these named bots. Consult only when needed to make a decision; no discussion step is mandatory. Give concrete responsibilities, exact accessible paths and acceptance checks. End your turn after assigning; busy teammates queue and results automatically resume you. When they return, finish the requested verification and give the user one final answer. Native helper names are not evidence that an Sandbase bot teammate participated. Plain @mentions are only for conversational replies in this room.",
...
function dispatchConnectorResume(entry: { botId: string; threadId: string; resumeKey: string; labels: string[] }) {
  ...
  const prompt = `Sandbase bot connection update: the user securely connected ${names}. Continue the task that paused for this connection. Do not ask them to connect it again.`;
...
function credentialDesktopHandoff(label: string): string {
  return `Securely provide the ${label} from Sandbase bot on your phone or computer. It is never added to chat.`;
}
...
function dispatchSecretResume(entry: SecretResumeEntry) {
  ...
  const prompt =
    entry.outcome === "provided"
      ? `Sandbase bot credential update: the user securely provided ${entry.label}. Continue the task that paused for it. You do not receive the secret and must not ask them to paste it into chat.`
      : `Sandbase bot credential update: the user declined to provide ${entry.label}. Continue without it if possible, or briefly explain the limitation. Do not ask them to paste it into chat.`;
  if (owner.group ? owner.bot.busy : threadBusy(entry.botId, entry.threadId) || activeGroupTurnForBot(entry.botId)) {
  ...
  try {
    ...
    if (path === "/api/settings/custom-domain") {
      ...
      if (method === "POST" || method === "DELETE") {
        if (DESKTOP_MANAGED) return json(res, 409, { error: "Custom domains are configured on a self-hosted Sandbase bot server, not the desktop companion." });
        if (!/^application\/json\b/i.test(String(req.headers["content-type"] ?? ""))) {
    ...
    if (path.startsWith("/api/internal/")) {
      ...
      if (path === "/api/internal/computer/select" && (method === "GET" || method === "POST")) {
        ...
        return json(res, 200, { status: "pending", surface: option.surface,
          message: `End this turn now without using the previous computer tools. Sandbase bot will continue the original request on ${option.label} with a fresh tool connection.` });
      }
    ...
    // relay never has to expose the rest of Sandbase bot's control surface.
    ...
    const requirePinnedClientThread = (botId: string, threadId: unknown): void => {
      if (threadId === undefined &&
        ...
        throw Object.assign(new Error("This bot has more than one thread. Update the Sandbase bot app on this device, then choose a thread and try again."), { status: 409 });
      }
    };
    ...
    // to Sandbase bot's private attachment directory. This is deliberately not
    ...
    if (m && method === "POST") {
      ...
      if (auth.kind !== "loopback") {
        return json(res, 403, { error: `This only works on the computer running Sandbase bot. The memory folder there is ${workspacePath}`, workspacePath });
      }
    ...
    if (method === "POST" && instanceAction) {
      ...
      try {
        ...
        if (action === "install") {
          if (!(await registry.installRuntime(instanceId))) return json(res, 404, { error: "Installing this engine from Settings is not available on this server. Use the install command on the machine running Sandbase bot." });
          return json(res, 200, { instances: await describeInstances() });
        }
        ...
        if (changingBoxToken && currentBoxToken) {
          ...
          if (currentBoxInventory) {
            ...
            for (const recovery of boxRecovery) {
              ...
              // than an Sandbase bot name check.
