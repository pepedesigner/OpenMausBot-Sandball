...
interface BrowserLookupOptions {
  ...
  /** Count only the runtimes Sandbase bot itself configured (the explicit
...
export function browserEngineStatus(options: BrowserLookupOptions = {}): BrowserEngineStatus {
  ...
  if (bundle && (options.exists ?? existsSync)(bundle.directory)) {
    return { kind: "unavailable", reason: "The desktop browser bundle is incomplete. Reinstall or update Sandbase bot to repair it.", installable: false };
  }
  const platform = options.platform ?? process.platform;
