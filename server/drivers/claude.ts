...
export function claudeInheritWarning(env: NodeJS.ProcessEnv): ProviderSnapshot["warning"] | undefined {
  ...
  return {
    ...
    message:
      "OMB_CLAUDE_INHERIT_USER_CONFIG=1 is set on the Sandbase bot process, so every Claude bot also loads this " +
...
export function claudeCliUpdate(version: string | null, cli: string): ProviderSnapshot["update"] | undefined {
  ...
  const effects = [
    ...(missing.includes("--autocompact") ? ["no compaction window picked by Sandbase bot"] : []),
...
const DENY_TIMEOUT_NOTE =
  "Sandbase bot: nobody answered this permission request in time. Skip this action and finish what you can without it.";
const QUESTION_TIMEOUT_NOTE = "Sandbase bot: nobody answered in time. Use your best judgment and continue.";
const DUPLICATE_ASK_ID_NOTE = "Sandbase bot: duplicate ask id — skipping this request.";
...
function systemEndedReply(kind: Ask["kind"]): { behavior: AskBehavior; message: string } {
  return kind === "question"
    ? { behavior: "answer", message: "Sandbase bot: the turn is ending — wrap up." }
    : { behavior: "deny", message: "Sandbase bot: the turn ended" };
}
  ...
  async create(input: DriverCreateInput<ClaudeConfig>): Promise<ProviderInstance> {
    ...
    const sendTurn = async (turn: SendTurnInput, logicalTurnId?: string) => {
      ...
      if (turn.integrations?.localComputer) {
        ...
        // Host tools always route through Sandbase bot's permission broker.
