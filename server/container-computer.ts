...
//
// Sandbase bot owns only the sandbox boundary: image preparation, container
...
function statusProblem(status: ContainerComputerStatus): string | null {
  ...
  if (!status.managed) return "The existing container was not created by Sandbase bot; recreate it";
 ...
 * versions. An older Sandbase bot container must stay removable (and eligible
...
export async function containerComputerStatus(
  ...
  try {
    ...
    // The prepared Sandbase bot derivative has not been built yet.
  }
...
export async function containerComputerAction(
  ...
  if (action === "remove" && !before.managed) {
    throw Object.assign(
      new Error(
        `The existing container named ${target.containerName} was not created by Sandbase bot; remove it manually in ${runtime}`,
      ),
      { status: 409 },
    );
  }
