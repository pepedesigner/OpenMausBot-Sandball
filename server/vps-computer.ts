...
function statusProblem(status: VpsComputerStatus): string | null {
  ...
  if (!status.image) return `Prepare the pinned Sandbase bot Cua image on the VPS (Driver ${CUA_DRIVER_VERSION})`;
  if (status.container === "missing") return "No Sandbase bot container exists for this bot on the VPS";
  if (!status.imageMatches) return "The VPS container uses an incompatible or untrusted Sandbase bot image";
  if (!status.managed) return "The VPS container name is occupied by a container Sandbase bot did not create";
  ...
  if (status.security === "unsafe") return "The VPS container is missing Sandbase bot safety limits";
  if (status.container === "stopped") return "The Sandbase bot VPS container is stopped";
  ...
  const operation = async () => {
    ...
    try {
      ...
      if (action === "provision") {
        ...
        // never docker-rm a container Sandbase bot did not create, even one
        // squatting on our name.
        if (before.container === "missing") return before;
        if (!before.managed) {
          throw Object.assign(
            new Error("The VPS container name is occupied by a container Sandbase bot did not create — remove it on the VPS yourself"),
            { status: 409 },
          );
        }
