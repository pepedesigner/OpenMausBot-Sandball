...
/** Names used to route Sandbase bot's built-in MCP proxies and their ephemeral
...
function environmentNameError(name: string): string | null {
  ...
  if (isHarnessOwnedMcpEnvName(name)) {
    return `Environment variable “${name}” is reserved by Sandbase bot.`;
  }
  return null;
}
...
export function mcpServerNameError(name: string): string | null {
  ...
  if (RESERVED_MCP_NAMES.has(name)) return "That name is reserved by Sandbase bot.";
  return null;
}
