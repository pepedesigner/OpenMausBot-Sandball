...
object SharePolicy {
    ...
    fun friendlyMessage(error: Throwable, computerName: String?): String {
        ...
        if (api != null && isAmbiguousTransport(api)) {
            return computerName?.let(::offline) ?: "Couldn't reach your computer. Keep Sandbase bot open and Phone access on, then try again."
        }
        ...
        detail == "Unlock this phone, then try sharing again." ||
            ...
            (
                detail.startsWith("Couldn't reach ") &&
                    detail.endsWith(". Keep Sandbase bot open and Phone access on, then try again.")
                )
    ...
    fun notPaired(): String =
        "Open the Sandbase bot app once after updating. If this phone still isn't connected, pair it before sharing."
    ...
    fun imageSupportUnavailable(): String =
        "Update Sandbase bot on this computer before sharing images."

    fun offline(name: String): String =
        "Couldn't reach $name. Keep Sandbase bot open and Phone access on, then try again."
    ...
    fun unreadable(name: String): String =
        "Sandbase bot couldn't read $name. Try exporting it to Files first."

    fun sendTimedOut(): String = "Sending took too long. Check your connection and try again."

    fun generic(): String = "Sandbase bot couldn't send this. Please try again."

    fun pairingExpired(): String =
        "This phone's pairing has expired. Open Sandbase bot and pair it again."
