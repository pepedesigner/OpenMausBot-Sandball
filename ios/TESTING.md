...
```sh
git clone https://github.com/pepedesigner/Sandbase-bot
cd Sandbase bot
...
- **The firewall is prompting.** System Settings → Network → Firewall. Incoming
  connections to `node`/Sandbase bot must be allowed, or the phone reaches
...
1. **Pair.** In Sandbase bot → Settings → Phone, choose **Pair a phone**.
   ...
   **Connect**. The computer should also appear by name for the manual path:
   ...
   - If the list stays empty, check in this order:
     1. **Local Network permission.** iOS asks once, and a denial is
        permanent and silent. Settings → Sandbase bot → Local Network. If the
...
3. **In Sandbase bot → Settings → Phone:** with Phone access on, the panel now
