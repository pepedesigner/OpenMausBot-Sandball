...
public struct PairingRouteError: Error, LocalizedError, Equatable, Sendable {
    ...
    public var errorDescription: String? {
        ...
        return "Couldn’t reach this computer through any available route (\(routes)). Keep Phone access turned on in Sandbase bot, then try again."
    }
}
...
public struct CompanionClient: Sendable {
    ...
    /// Only the first response that identifies itself as Sandbase bot receives
        ...
        do {
            ...
            guard message?.localizedCaseInsensitiveContains(Self.alreadyDrainedQueueMessage) == true else {
                throw APIError.status(
                    code: 404,
                    message: "This computer is too old to take back a queued message. Update Sandbase bot on it."
                )
            }
        }
    }
