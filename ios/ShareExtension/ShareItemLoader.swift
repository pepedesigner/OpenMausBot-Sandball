...
enum ShareItemLoadingError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .appGroupUnavailable:
            return "Sandbase bot couldn't open its secure sharing folder. Open the app once, then try again."
        case .nothingSupported:
        ...
        case let .unreadable(name):
            return "Sandbase bot couldn't read \(name). Try exporting it to Files first."
        }
    }
}
