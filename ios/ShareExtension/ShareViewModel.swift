    ...
    private func friendlyMessage(for error: Error) -> String {
        if let apiError = error as? APIError {
            if apiError.isUnauthorized {
                return "This phone's pairing has expired. Open Sandbase bot and pair it again."
            }
            if isAmbiguousTransport(error) {
                return "Couldn't reach your computer. Keep Sandbase bot open and Phone access on, then try again."
            }
        }
        return (error as? LocalizedError)?.errorDescription
            ?? "Sandbase bot couldn't send this. Please try again."
    }
...
private enum ShareExtensionError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .notPaired:
            return "Open the Sandbase bot app once after updating. If this phone still isn't connected, pair it before sharing."
        case .noDestinations:
            return "There aren't any bots or channels to send this to yet. Create one on your computer first."
        case .imageSupportUnavailable:
            return "Update Sandbase bot on this computer before sharing images."
        case let .offline(name):
            return "Couldn't reach \(name). Keep Sandbase bot open and Phone access on, then try again."
        case .sendTimedOut:
