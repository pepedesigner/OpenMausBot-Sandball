...
enum MausPalette {
    /// src/lib/mascot.ts — MAUS_COLORS (ten sand colours, one per bot)
    ...
    static let faceInk = Color(hex: "#33261A")
...
final class MausFaceEngine {
    ...
    private func drawBody(in context: inout GraphicsContext, color: String, bodyId: String?, now: Date) {
        ...
        for ring in rings {
            ...
            path.closeSubpath()
            context.fill(path, with: .color(MausPalette.faceInk))
        }
        ...
        context.stroke(
            ...
            with: .color(MausPalette.faceInk),
...
extension MausPalette {
    ...
    /// `LinearGradient` directly. The lighting is the sand material's own: a 22%
    /// lift toward white, the base colour, then a 12% drop toward black.
    static func gradientStops(_ name: String) -> [Gradient.Stop] {
        let base = color(name)
        return [
            .init(color: base.mixed(with: .white, amount: 0.22), location: 0),
            .init(color: base, location: 0.62),
            .init(color: base.mixed(with: .black, amount: 0.12), location: 1),
        ]
    }
}
