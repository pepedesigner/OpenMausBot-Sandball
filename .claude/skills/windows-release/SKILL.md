...
description: Build and verify the Windows desktop build (NSIS installer + latest.yml) for the canonical Sandbase bot release and its legacy updater mirror. Use when cutting a release, shipping a new version to Windows users, or when a Windows user reports they are stuck on an old version. Windows only — does not cover the macOS dmg/notarization flow.
---

# Windows release

Ships `Sandbase bot-<version>-setup.exe` and its update feed to
[milind-soni/Sandbase bot](https://github.com/milind-soni/OpenMausBot/releases).
...
| File | Purpose |
|---|---|
| `Sandbase bot-<version>-setup.exe` | the installer |
| `latest.yml` | **the update feed** — see step 4 |
| `Sandbase bot-<version>-setup.exe.blockmap` | differential updates |
| `Sandbase bot-<version>-x64.zip` | portable, not used by the updater |
...
- `app-update.yml` must point at `milind-soni/Sandbase bot` and, while the
...
2. The chat window renders (not the error page). Server logs land in
   `%APPDATA%\Sandbase bot\logs\server.log`.
...
```powershell
Copy-Item release/Sandbase bot-<version>-setup.exe release/Sandbase-bot-setup.exe
gh release upload v<version> --repo milind-soni/Sandbase bot `
  release/Sandbase bot-<version>-setup.exe `
  release/Sandbase-bot-setup.exe `
  release/Sandbase bot-<version>-setup.exe.blockmap `
  release/latest.yml
```
...
- **`Sandbase bot-<version>-setup.exe`** is what `latest.yml` references by name and
  sha512. The auto-updater downloads exactly this.
- **`Sandbase-bot-setup.exe`** is a byte-identical copy that gives the README's
  `/releases/latest/download/Sandbase-bot-setup.exe` button a stable URL. This
  mirrors `Sandbase-bot.dmg` sitting beside `Sandbase bot-<version>.dmg`.
