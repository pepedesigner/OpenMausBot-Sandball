...
Add OpenCode Go as a first-class Sandbase bot engine by running the maintained
OpenCode CLI over its official Agent Client Protocol (ACP) interface and selecting
models from the `opencode-go/*` provider.

The first implementation should reuse Sandbase bot's ACP runtime rather than call
...
provide inference; Sandbase bot does not currently have a provider-independent
...
- OpenCode accepts an `OPENCODE_API_KEY` environment variable. Sandbase bot should
...
the process lifecycle Sandbase bot uses for Grok and Gemini.
...
- refresh on the existing instance re-probe path so upstream additions do not
  require restarting Sandbase bot.
...
- load that session on the next Sandbase bot turn;
...
- concurrent Sandbase bot threads without shared ACP or credential state.
...
- OpenCode Go is shown as unavailable until both its CLI and credential are
  present; Sandbase bot never selects it as a runnable default otherwise.
- The setup flow works without restarting Sandbase bot and never silently executes
  an installer.
...
6. Can the existing ACP usage metadata distinguish cumulative session totals from
   per-turn usage so Sandbase bot does not double-count it?
