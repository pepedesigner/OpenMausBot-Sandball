...
OpenMausMobile is a companion for an Sandbase bot service chosen and operated
by the user. Local Wi-Fi and Tailscale connections work without an Sandbase bot
...
- The computer remains the source of bots, transcripts, approvals, credentials,
  SQLite data, and screen images. Sandbase bot's hosted control plane does not
...
- If the desktop user enables optional hosted access, Sandbase bot stores the
...
- The optional HTTPS route is proxied by Cloudflare to an outbound-only
  ...
  but are not written to the Sandbase bot control-plane database. Cloudflare may
  process IP addresses and connection/request metadata as Sandbase bot's service
...
- Connector tokens stay in the desktop operating system's encrypted credential
  ...
  database.
- When a user explicitly chooses Sandbase bot from another app's Share sheet, or
...
Revoking the phone in Sandbase bot's Companion settings invalidates that device
credential. Transcript deletion is controlled by the Sandbase bot installation
...
[Sandbase bot Support](https://github.com/pepedesigner/Sandbase-bot/issues) request
...
[Sandbase bot Support](https://github.com/pepedesigner/Sandbase-bot/issues).
