...
```
"List the other bots (agents) in your Sandbase bot section, with their model and whether they're busy.
```

to:

```
"List the other bots (agents) in your Sandbase bot section, with their model and what each is doing right now (available, working, waiting on the user, not responding, or unavailable).
```
