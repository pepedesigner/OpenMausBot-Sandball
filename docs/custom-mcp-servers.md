...
Sandbase bot saves a new server switched off. Use **Test** to start the command
...
`OMB_CLAUDE_INHERIT_USER_CONFIG=1` on the Sandbase bot process remains the full
...
window with `--autocompact` (2.1.122+). Sandbase bot reads `claude --version`
...
by hand, restart Sandbase bot. Every bot whose engine can mount custom MCP
...
- **Testing is bounded.** A command is stopped after the handshake (or eight
  ...
  inherits none of Sandbase bot's workspace or provider credentials; only the
