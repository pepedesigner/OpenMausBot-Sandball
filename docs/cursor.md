...
Cursor is an optional Sandbase bot engine. Sandbase bot runs the official
...
3. Confirm `cursor-agent --version` works. The binary installs to `~/.local/bin` by
   default; Sandbase bot already looks there when launched from a GUI.
...
CLI also implements ACP `session/set_model`, Sandbase bot pins the same id over
...
does not provide a bot approval level. Sandbase bot app turns always provide
one: both **Ask for approval** and **Approve for me** launch Cursor without
`--force`, then Sandbase bot handles its permission requests according to the
bot's current level.
