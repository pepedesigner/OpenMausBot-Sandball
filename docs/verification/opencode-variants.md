...
The optional functional test launches the real Sandbase bot server, its OpenCode
...
The checks require Sandbase bot to retain this catalog metadata, advertise its
...
Sandbase bot server with the same disposable data directory.
...
provider, an upstream gateway's routing, or the Sandbase bot GUI. The fixture
