# Sandbase bot MCP server

The Sandbase bot desktop app includes a local stdio MCP server. It lets another MCP client coordinate your
Sandbase bot team while the desktop app and its harness are running.
...
Start Sandbase bot, then configure the MCP client to run:
...
```json
{
  "mcpServers": {
    "openmausbot": {
      "command": "pnpm",
      "args": ["--dir", "/absolute/path/to/Sandbase bot", "mcp"]
    }
  }
}
```
...
1. In Sandbase bot, open **Settings → Phone → Set up a phone** and reveal the
...
```json
{
  "mcpServers": {
    "openmausbot": {
      "command": "/Applications/Sandbase bot.app/Contents/MacOS/Sandbase bot",
      "args": ["/Applications/Sandbase bot.app/Contents/Resources/server/mcp-server.js"],
...
On Windows, use the installed `Sandbase-bot.exe` as `command`, the adjacent
`resources\\server\\mcp-server.js` as the argument, and the same `ELECTRON_RUN_AS_NODE=1` environment value.
The usual per-user install is under `%LOCALAPPDATA%\\Programs\\Sandbase bot`.

On Ubuntu `.deb` installs, the executable is normally `/opt/Sandbase bot/openmausbot` and the script is
`/opt/Sandbase bot/resources/server/mcp-server.js`. Use the same environment value.

## Connection discovery

With no configuration, the MCP process probes Sandbase bot's three desktop ports (`8799`, `18799`, and `28799`)
and accepts only a health response that identifies itself as Sandbase bot. This handles the desktop's normal
