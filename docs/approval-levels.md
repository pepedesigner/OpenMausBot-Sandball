...
provider's own permission modes, passed through. Sandbase bot does not judge an
...
| **Custom (`config.toml`)** | Codex only. Sandbase bot reads and reapplies the effective approval and sandbox settings from your Codex configuration. |
...
ends. Sandbase bot keeps no standing grant for a provider's tool. It is not
...
Native modes require a CLI version that supports them; Sandbase bot does not
silently substitute unrestricted access when a mode is rejected.

### Read-only integration tools

Sandbase bot's built-in agents MCP server describes nine scoped reads with
...
provider's session. Its quieter default is Full access; Sandbase bot keeps its
