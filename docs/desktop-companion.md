...
Every Sandbase bot desktop build can play either role:
...
The HTTPS address uses the host's managed outbound tunnel. TLS is verified by the operating system, and the client does not need Tailscale. HTTPS is intentionally restricted to Sandbase bot-managed companion names so a typo cannot redirect a paired-device token to an unrelated site.
