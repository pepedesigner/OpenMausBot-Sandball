...
The iOS app is a thin, native client for the Sandbase bot instance running on
...
an Sandbase bot account of its own.
...
The Mac must be running Sandbase bot and must not be asleep. Desktop
...
to reach. Sandbase bot then places the computer's MagicDNS name in that
...
Tailscale is optional. The direct path does not use an Sandbase bot-operated
...
The Sandbase bot control plane stores account and installation metadata plus
...
The Mac must remain awake with Sandbase bot running for chats, approvals, and
...
An Sandbase bot account is not required for nearby, manual, or Tailscale
