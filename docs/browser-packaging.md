...
AppArmor policy for the root-owned browser executable under `/opt/Sandbase bot`.
...
On macOS, use `/absolute/Sandbase bot.app/Contents/Resources`. On Linux, run as
an unprivileged user against the installed `.deb` at
`/opt/Sandbase bot/resources`. This check creates its own empty home and local
...
Windows uses an explicitly identified Sandbase bot build of agent-browser
