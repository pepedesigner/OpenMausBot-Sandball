# Deploy Sandbase bot on a VPS

From a blank Linux server to Sandbase bot running on it around the clock, reachable from your laptop, the desktop app and your phone, with your bots working while every laptop is closed. It assumes nothing beyond being able to open a terminal and paste commands. About twenty minutes, most of it waiting.
...
|---|---|---|---|
| **A. Public address, no domain** (`serve --tunnel`) | an Sandbase bot account (email code) | anyone with a pairing code, over HTTPS | the fastest path; a phone on cellular |
...
`setup` connects an AI provider; it is separate from the Sandbase bot account.
...
3. **Get the deploy files and set the name:**
   ...
   ```sh
   git clone https://github.com/pepedesigner/Sandbase-bot && cd Sandbase bot/deploy
   cp .env.example .env
...
The npm Sandbase bot package does not install model engine CLIs. For paths A and C,
...
`~/.claude`), separately from Sandbase bot's `~/.openmausbot`. Keep that home when
...
[Unit]
Description=Sandbase bot server
...
- **Path B:** `cd Sandbase bot/deploy && docker compose pull omb && docker compose up -d`.
...
`Sandbase bot/deploy`, stop the app before archiving; `deploy_data` is the default
