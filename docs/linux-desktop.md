...
Sandbase bot has an Ubuntu 24.04 LTS x86_64 desktop beta. The Electron package embeds the harness server, so
...
- The native Electron window and embedded Sandbase bot server on GNOME Xorg and GNOME Wayland.
...
progress in [issue #29](https://github.com/pepedesigner/Sandbase-bot/issues/29) and the safety hold in
[issue #345](https://github.com/pepedesigner/Sandbase-bot/issues/345). Bundled
CUA supply-chain work is tracked in [issue #113](https://github.com/pepedesigner/Sandbase-bot/issues/113). Xorg is tracked in
[issue #79](https://github.com/pepedesigner/Sandbase-bot/issues/79), and guarded GNOME/Wayland support in
[issue #109](https://github.com/pepedesigner/Sandbase-bot/issues/109).

## Download packages

Choose one Ubuntu 24.04 x86_64 package from the latest release:

- [Debian package (`Sandbase-bot-amd64.deb`)](https://github.com/milind-soni/OpenMausBot/releases/latest/download/Sandbase-bot-amd64.deb) — recommended; APT installs its desktop dependencies and configures the bundled bot browser's sandbox.
- [Portable AppImage (`Sandbase-bot.AppImage`)](https://github.com/milind-soni/OpenMausBot/releases/latest/download/Sandbase-bot.AppImage) — does not install system files.
...
- `release/Sandbase bot-<version>-amd64.deb`
- `release/Sandbase bot-<version>-x86_64.AppImage`
...
```sh
sudo apt install ./Sandbase-bot-amd64.deb
```

Then open **Sandbase bot** from the GNOME application launcher. To remove it:
...
```sh
chmod +x release/Sandbase bot-*-x86_64.AppImage
./release/Sandbase bot-*-x86_64.AppImage
```

For a downloaded release AppImage, use `Sandbase-bot.AppImage` in place of the versioned path above.
...
Applications launched from GNOME do not inherit the same interactive shell `PATH` as a terminal. Sandbase bot
...
```sh
OMB_EXTRA_PATH=/your/custom/bin ./release/Sandbase bot-*-x86_64.AppImage
```

Restart Sandbase bot after installing or signing in to a CLI.
...
24.04 GNOME Wayland session. Sandbase bot detects Wayland before XWayland when both `WAYLAND_DISPLAY` and
...
automatically. Sandbase bot does not capture screen audio, remember the selected monitor after restart, or
...
starts with `--no-overlay`, so Cua's decorative full-screen X11 cursor surface is never created. Sandbase bot also
...
The Xorg runtime uses private sockets, standard permission mode, per-action Sandbase bot approvals,
...
then runs the same smoke against `/opt/Sandbase bot/openmausbot`. These lanes prove the embedded server and UI are
...
Run the CLI directly in a terminal, finish its sign-in flow, then restart Sandbase bot. If it lives outside the
...
```sh
chmod +x Sandbase bot-*-x86_64.AppImage
file Sandbase bot-*-x86_64.AppImage
```
