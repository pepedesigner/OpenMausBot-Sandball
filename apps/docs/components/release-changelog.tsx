...
const RELEASE_REPOSITORIES = [
  'milind-soni/Sandbase bot',
  'milind-soni/openmausbot-releases',
] as const;
const RELEASES_PER_PAGE = 100;
const MAX_RELEASE_PAGES = 10;
const LEGACY_DRAFT_NOTES =
  /^Draft assembled by the release workflow from milind-soni\/Sandbase bot@([0-9a-f]{40})\. Edit these notes, then publish\.\s*$/i;
...
function releaseNotes(release: GitHubRelease) {
  ...
  return `This build predates curated release notes. [View its source commit (${commit.slice(0, 7)})](https://github.com/pepedesigner/Sandbase-bot/commit/${commit}).`;
}
