...
Open **Connected apps** from the sidebar, choose a service, and complete its authorization in your normal browser. Sandbase bot turns the resulting connection into tools that supported agent engines can use.

<ProductScreenshot name="docs-connected-apps" alt="The clean Sandbase bot connected-app marketplace" caption="Search the marketplace or review every account from the Connected tab." />

## Official packaged app

The signed desktop app registers its own installation with Sandbase bot's managed connected-apps service. You do **not** need to find or paste a Composio key.
...
The official managed Sandbase bot service keeps X disabled until it owns an appropriate X OAuth app. Older Sandbase bot tasks that refer to the connector as `x` continue to work; Sandbase bot translates that alias to Composio's `twitter` toolkit automatically.

## Multiple accounts

Connect more than one account for the same toolkit and give each a clear alias such as `work`, `personal`, or `client-acme`. When several accounts could perform an action, Sandbase bot requires explicit selection instead of silently replacing the first connection.
