...
for (const appName of ["openmausbot", "Sandbase bot"]) {
...
try {
  ...
  if (!String(title).includes("Sandbase bot")) throw new Error(`unexpected renderer title: ${title}`);
  ...
  if (sessionBlocked) {
    ...
    const activeUserData = ["openmausbot", "Sandbase bot"]
    ...
    const userData = ["openmausbot", "Sandbase bot"]
      .map((name) => path.join(xdgConfig, name))
      .find((directory) => existsSync(path.join(directory, "cua-connection.json")));
