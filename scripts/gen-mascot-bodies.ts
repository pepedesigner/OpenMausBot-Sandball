...
/** The tightest outline in the catalog: its face size sets the floor every other body is clamped to. */
const FLOOR_ID = "cursor"

/** The body a bot wears when it has not chosen one — the shipped mascot. */
const SHIPPED_ID = "circle"
...
export const DEFAULT_MASCOT_BODY: MascotBodyId = ${quote(SHIPPED_ID)};
...
enum MausBodies {
    ...
    /// The shipped mascot, and the fallback for any unrecognised value.
    static let defaultID = ${quote(SHIPPED_ID)}
...
internal object MausBodies {
    ...
    /** The shipped mascot, and the fallback for any unrecognised value. */
    const val DEFAULT_ID: String = ${quote(SHIPPED_ID)}
...
function main(): void {
  ...
  const floorBody = solved.find(s => s.def.id === FLOOR_ID)
  if (!floorBody) throw new Error(`the catalog has no ${FLOOR_ID} body to anchor its face size to`)
  ...
  if (shared < floorBody.scale) {
    const tighter = solved
      .filter(s => s.scale < floorBody.scale)
      .map(s => `${s.def.id} (${s.scale})`)
      .join(", ")
    throw new Error(
      `the shared face scale ${shared} is below ${FLOOR_ID}'s own ${floorBody.scale}: ` +
  ...
  console.log(`shared face scale ${shared} (${FLOOR_ID}'s own floor is ${floorBody.scale})`)
