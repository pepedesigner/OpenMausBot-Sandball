...
if (process.versions.electron && process.argv.includes(flag)) {
  ...
  app.whenReady().then(async () => {
    ...
    await until(() => evaluate("document.body.textContent.includes('Welcome to Sandbase bot')"), "normal optional welcome flow");
    assert.equal(await evaluate(`Boolean(${button("Sign in with your organisation")})`), false);
    ...
    await until(() => evaluate("document.body.textContent.includes('Welcome to Sandbase bot')"), "local choice survives recreated renderer");
