...
try {
  ...
  if (status.startsWith("ii")) fail("refusing to replace a pre-existing Sandbase bot installation");
} catch (error) {
...
const legacyApp = path.join(legacyRoot, "opt", "Sandbase bot");
...
try {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "Version: 0.1.7",
      "Architecture: amd64",
      "Maintainer: Sandbase bot CI <ci@openmausbot.invalid>",
      "Description: Legacy Sandbase bot directory-mode upgrade fixture",
      "",
    ].join("\n"),
    { mode: 0o644 },
  );
  ...
  for (const directory of ["/opt/Sandbase bot", "/opt/Sandbase bot/resources"]) {
  ...
  for (const directory of [
    "/opt/Sandbase bot",
    "/opt/Sandbase bot/resources",
    "/opt/Sandbase bot/resources/cua-linux-x64",
  ]) {
  ...
  for (const executable of ["cua-driver", "cua-cursor-theme"]) {
    const file = path.join("/opt/Sandbase bot/resources/cua-linux-x64", executable);
  ...
  const chromiumSandbox = "/opt/Sandbase bot/chrome-sandbox";
