...
function requireUpdaterTarget(resources, label) {
  ...
  if (!/^owner: milind-soni$/m.test(update) || !/^repo: Sandbase bot$/m.test(update)) {
    fail(`${label} app-update.yml does not point at milind-soni/Sandbase bot`);
  }
}
...
try {
  ...
  const debAppRoot = path.join(extracted, "opt", "Sandbase bot");
  ...
  for (const expected of [
    "Name=Sandbase bot",
    "Exec=/opt/Sandbase bot/openmausbot %U",
    "Icon=openmausbot",
