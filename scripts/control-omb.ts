...
export const HELP = `control-omb — verify a running Sandbase bot instance through its shared MCP core
...
function configuredUrl(raw: unknown, env: NodeJS.ProcessEnv, requiredForMutation: boolean): string | undefined {
  ...
  if (!explicit) {
    if (requiredForMutation) {
      throw new ControlOmbError(
        "mutating commands require an explicit Sandbase bot instance",
