...
import { SandFilter } from "./SandMaterial"
import {
  EXPRESSIONS,
  EXPRESSION_COUNT,
  FACE_BOX,
...
// The generator solves each body's face placement once; `MASCOT_BODIES` is the
// single source of truth desktop and iOS both build from. Its entries carry one
// extra field (`id`) that `CursorSilhouette` does not, so the default is derived
// here rather than assigned directly — and the default is the shipped mascot,
// the sand ball.
const { id: _defaultBodyId, ...defaultSilhouette } = MASCOT_BODIES[DEFAULT_MASCOT_BODY]
export const DEFAULT_SILHOUETTE: CursorSilhouette = defaultSilhouette

/** The pack's default green, as the sand material's own highlight/base/shadow
 * stops (a 22% lift toward white, a 12% drop toward black). */
export const DEFAULT_GRADIENT: [string, string, string] = ["#69C08E","#3FAE6E","#379961"]
...
export interface CursorAvatarProps {
  ...
  eyeColor?: string
  /** Render the body as a ball of loose sand instead of a smooth vector fill. */
  sand?: boolean
  title?: string | null
  className?: string
  ...
  function CursorAvatar(
    {
      ...
      // The material's own eye ink: dark, like the face carved into a sand
      // sculpture. White eyes were right on the dark neon bodies this app
      // shipped with; on sand they wash out.
      eyeColor = "#33261A",
      sand = true,
      title,
      className,
    ...
    return (
      ...
      >
        <defs>
          {/* Lighting off-centre and low, the way the sand material's own radial
              gradient is: the highlight sits up and to the left of the middle,
              so the body reads as a lit sphere rather than a shaded disc. */}
          <radialGradient id={`${uid}-grad`} cx="38%" cy="32%" r="75%">
            <stop offset="0%" stopColor={gradient[0]} />
            <stop offset="62%" stopColor={gradient[1]} />
        ...
        <g transform={flip ? `translate(${FACE_BOX} 0) scale(-1 1)` : undefined}>
          ...
          <g ref={bodyGroup}>
          <g ref={bodyContent}>
          {/* The sand filter hangs on a wrapper that carries no transform of its own,
              so the noise is generated in face-box units whichever body is worn —
              the outlines arrive in their own artwork spaces (200, 512, …). */}
          <g filter={sand ? `url(#${uid}-sand)` : undefined}>
