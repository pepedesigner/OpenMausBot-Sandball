...
function LocalVmPane({
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      if (!bridge) {
        setError("The two-desktop workspace requires the Sandbase bot desktop app.");
        return;
      }
      try {
        ...
        setError(
          cause instanceof Error && cause.message === "layout-unavailable"
            ...
            : "Sandbase bot could not connect this Local VM viewer.",
        );
      }
    };
  ...
  const updateLayout = useCallback(() => {
    ...
    void bridge
      ...
      .catch(() => setError("Sandbase bot could not position this Local VM viewer."));
...
export function LocalVmWorkspace({
  ...
  const handBack = useCallback(async () => {
    ...
    try {
      ...
      setControlError("Sandbase bot could not hand control back. The workspace stayed open.");
      return false;
    } finally {
