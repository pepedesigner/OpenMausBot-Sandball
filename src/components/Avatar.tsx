 ...
 * shadow), lit like the shipped sand ball: a soft dome highlight rather
 * than a sheen, so the body reads as a matte sphere of sand. The two
 * amounts are the material's own — a 22% lift toward white and a 12%
 * drop toward black.
 */
const gradientFor = (color: MausColor): [string, string, string] => {
  const fill = MAUS_COLORS[color] ?? MAUS_COLORS.green;
  return [mix(fill, "#ffffff", 0.22), fill, mix(fill, "#000000", 0.12)];
};
