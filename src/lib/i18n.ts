 ...
 *
 * `{app}` is always available, filled from the deployment's brand: copy that
 * names the product does not have to thread the name through every call site,
 * and a white-labelled brand.json renames those strings too. */
export function t(key: LocaleKey, params?: Record<string, string | number>): string {
  ...
  return template.replace(/\{(\w+)\}/g, (match, name: string) => {
    ...
    return name === "app" ? brand().name : match;
  });
}
