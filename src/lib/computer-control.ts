...
>(input: {
  ...
  if (action === "take" && syncNativeBrowser && !(await setNativeBrowserControl(true))) {
    throw new Error("Sandbase bot could not pause this bot's browser safely");
  }
  const snap = await requestControl(action);
  if (snap.held !== (action === "take")) {
    throw new Error(`Sandbase bot could not ${action === "take" ? "confirm" : "release"} computer control`);
  }
