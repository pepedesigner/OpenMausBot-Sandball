...
/**
 * The mascot palette — ten sand colours, one per bot.
 *
 * Earthy and desaturated on purpose: a bot is a ball of coloured sand, so the
 * hues are pulled toward the ground rather than the neon the app shipped with.
 * The names are the wire format (bots persist one of them), so they stay put
 * even where the hue has moved — `green` is a moss sand, not the old emerald.
 *
 * Lightness is tuned to sit where the old palette did, because these are also
 * the mention colours: `--mention-color` draws bot names in chat, and both the
 * dark and the light skins have to keep them legible.
 */
