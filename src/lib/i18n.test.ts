...
describe("t", () => {
  ...
  it("fills {app} from the brand, with no params passed", () => {
    // Copy that names the product writes {app} rather than the name, so a
    // white-labelled brand.json renames those strings too. The call site is
    // not required to thread the name through; an explicit param still wins.
    locales["zz"] = { "onboarding.welcome": "Willkommen bei {app}" };
    try {
      setLocale("zz");
      expect(t("onboarding.welcome")).toBe("Willkommen bei Sandbase bot");
      expect(t("onboarding.welcome", { app: "Something else" })).toBe("Willkommen bei Something else");
    } finally {
      delete locales["zz"];
      setLocale("en");
    }
