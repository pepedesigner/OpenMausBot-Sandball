...
test("a protected directory spelled in another case is still refused", async t => {
  ...
  await mkdir(path.join(dir, "Sandbase bot"));
  await writeFile(path.join(dir, "Sandbase bot", "credentials.bin"), "credential blob");
  grant.protectedPaths = [path.join(dir, "Sandbase bot")];
  folder.write = true;
  await assert.rejects(run({ action: "read_file", path: "Sandbase bot/credentials.bin" }), /Desktop credentials/);
