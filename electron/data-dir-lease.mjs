...
function parseRecord(path, invalidMessage, validate) {
  ...
  try {
    ...
    throw leaseError("Sandbase bot cannot read the data-directory lease; refusing to start to protect its state.", error);
  }
  let record;
  try {
...
function readOwner(path) {
  return parseRecord(
    path,
    "The Sandbase bot data-directory lease is invalid; refusing to start to protect its state.",
    isLeaseOwner,
  );
}
...
function readReaper(path, targetToken) {
  return parseRecord(
    path,
    "The Sandbase bot stale-lease recovery record is invalid; refusing to start to protect its state.",
...
function processIsAlive(pid) {
  try {
    ...
    throw leaseError("Sandbase bot could not verify the data-directory lease owner; refusing to start.", error);
  }
}
...
function publishRecord(path, record, prepareMessage, acquireMessage) {
  ...
  try {
    ...
    unlinkExact(candidatePath, "Sandbase bot could not remove its lease candidate.");
  }
}
...
function claimReaperAuthority(leasePath, expected) {
  ...
  for (let generation = 0; generation < MAX_REAPER_GENERATIONS; generation += 1) {
    ...
    if (publishRecord(
      reaperPath,
      candidate,
      "Sandbase bot could not prepare stale-lease recovery.",
      "Sandbase bot could not safely recover the stale data-directory lease.",
    )) return true;
    ...
    if (current.host !== candidate.host) {
      throw leaseError(
        `A stale Sandbase bot data-directory lease is being recovered on another machine. Recovery record: ${JSON.stringify(reaperPath)}.`,
      );
    }
    if (ownerIsAlive(current)) return false;
    reaperPath = successorReaperPath(leasePath, expected.token, current.token);
  }
  throw leaseError("Sandbase bot could not recover the stale data-directory lease after repeated interrupted attempts.");
}
...
function retireDeadOwner(leasePath, expected) {
  ...
  if (current.host !== hostname()) {
    throw leaseError(
      `The stale Sandbase bot data-directory lease changed ownership to another machine. Lease record: ${JSON.stringify(leasePath)}.`,
    );
  }
  if (ownerIsAlive(current)) return false;
  unlinkExact(leasePath, "Sandbase bot could not retire the stale data-directory lease.");
  return true;
}

function validateDataDir(dataDir) {
  if (typeof dataDir !== "string" || dataDir.trim().length === 0 || /[\r\n\0]/.test(dataDir)) {
    throw leaseError("Sandbase bot cannot lease an invalid data directory.");
  }
  return dataDir;
}
...
function prepareDataDir(dataDir, legacyDataDir) {
  ...
  try {
    ...
    throw leaseError("Sandbase bot cannot create its data directory.", error);
  }
...
function assertNoLiveDelegatedChild(dataDir) {
  ...
  if (child.host !== hostname()) {
    throw leaseError(
      `This Sandbase bot data directory still has a delegated server on another machine. Delegated server lease: ${JSON.stringify(childLeasePath)}.`,
    );
  }
  if (ownerIsAlive(child)) {
    throw leaseError(
      `Sandbase bot's previous server process ${child.pid} is still shutting down. Try again shortly.`,
    );
  }
}
...
function consumeChildCapability(environment) {
  ...
  try {
    ...
    throw leaseError("Sandbase bot could not consume its private desktop lease delegation.", error);
  }
  if (environment[CHILD_LEASE_ENV] !== undefined) {
    throw leaseError("Sandbase bot could not consume its private desktop lease delegation.");
  }
  return value;
}
...
function validateChildDelegation(dataDir, encoded) {
  ...
  const invalid = () => leaseError("The Sandbase bot desktop lease delegation is invalid; refusing to start to protect its state.");
...
function acquireDataDirLeaseInternal(dataDir, options = {}) {
  ...
  try {
    ...
    throw leaseError("Sandbase bot cannot prepare its data-directory lease.", error);
  }
    ...
    for (;;) {
      try {
        ...
        if (error?.code !== "EEXIST") {
          throw leaseError("Sandbase bot cannot acquire its data-directory lease.", error);
        }
      }
      ...
      if (current.host !== owner.host) {
        throw leaseError(
          `This Sandbase bot data directory is already owned by a process on another machine. Lease record: ${JSON.stringify(leasePath)}.`,
        );
      }
      if (ownerIsAlive(current)) {
        throw leaseError(
          `Sandbase bot is already using this data directory (process ${current.pid}). Close the other instance first. If no Sandbase bot is running, delete this lease record and start again: ${JSON.stringify(leasePath)}.`,
        );
      }
      if (!retireDeadOwner(leasePath, current)) {
        throw leaseError("A stale Sandbase bot data-directory lease is already being recovered; try again shortly.");
      }
    }
  } finally {
    unlinkExact(candidatePath, "Sandbase bot could not remove its lease candidate.");
  ...
  if (!acquired) throw leaseError("Sandbase bot could not acquire its data-directory lease.");
  let released = false;
  return Object.freeze({
    ownerPid: owner.pid,
    delegated: false,
    release() {
      ...
      if (!current || current.pid !== owner.pid || current.host !== owner.host || current.token !== owner.token) {
        throw leaseError("Sandbase bot will not release a data-directory lease owned by another process.");
      }
      try {
        unlinkSync(leasePath);
      } catch (error) {
        throw leaseError("Sandbase bot could not release its data-directory lease.", error);
      }
      released = true;
      return true;
    },
    utilityServerLeaseEnvironment() {
      if (released) throw leaseError("Sandbase bot cannot delegate a released data-directory lease.");
      return Object.freeze({ [CHILD_LEASE_ENV]: capabilityFor(owner) });
    },
  });
}

/** Claim exclusive ownership of one persistent Sandbase bot data directory. */
