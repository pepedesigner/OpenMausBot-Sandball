...
//    EmbeddedCuaDriverHost so TCC grants attribute to Sandbase bot and the
//    driver inherits them. One prompt, named Sandbase bot, out of the box.
...
async function attachStandalone(signal) {
  ...
  if (!(await socketAlive(STANDALONE_SOCKET))) {
    ...
    // machine already granted — instead of the freshly signed Sandbase bot.
...
async function startEmbedded(binary, signal) {
  ...
  // Sandbase bot rather than to a terminal or helper process.
  if (process.platform === "darwin") {
    ...
    if (!sdk.hasRequiredMacOSPermissions(permissionStatus)) {
      ...
      throw new Error(`${missing || "macOS permissions"} required; grant access in System Settings and restart Sandbase bot`);
    }
  }
