...
function workspaceWindowTitle(state, companion) {
  if (companion) return `Sandbase bot — Connected to: ${companion.serverName} (${new URL(companion.endpoint).host})`;
  const active = activeEnvironment(state);
  return active ? `Sandbase bot — Hosted: ${active.name} (${new URL(active.origin).host})` : "Sandbase bot";
}
