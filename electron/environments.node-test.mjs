...
test("native window identity distinguishes hosted HTML, companion data, and the local workspace", () => {
  ...
  assert.equal(env.workspaceWindowTitle(state), "Sandbase bot — Hosted: Old team (old.example)");
  assert.equal(env.workspaceWindowTitle(state, { serverName: "Office", endpoint: "https://c-office.openmausbot.com" }), "Sandbase bot — Connected to: Office (c-office.openmausbot.com)");
  assert.equal(env.workspaceWindowTitle(env.withActive(state, "local")), "Sandbase bot");
});
