...
if (!app.requestSingleInstanceLock()) {
  console.log("[desktop] Sandbase bot is already running — focusing that window");
  process.exit(0);
}
...
const serverSupervisor = createServerSupervisor({
  ...
  onExhausted() {
    ...
    dialog.showErrorBox(
      ...
      "Automatic recovery could not restart the background server. Quit and reopen Sandbase bot to try again. Interrupted chat turns were not resent.\n\n" +
...
// goes to server.log in the OS log dir (~/Library/Logs/Sandbase bot on macOS,
// Console.app-visible; %APPDATA%\Sandbase bot\logs on Windows), which is also
...
async function runCompanyBackup(kind, input, scheduled = null) {
  ...
  try {
    ...
    if (status.pendingRestore) {
      ...
      throw new Error("Restart Sandbase bot to finish the pending restore before starting another backup operation.");
    }
...
function buildErrorPage({ allPortsOccupied }) {
  ...
  const reason = allPortsOccupied
    ? "Every Sandbase bot port answered health checks from another process — likely a second copy of the app, or another program on ports 8799–28799. Quit that program, then quit and reopen Sandbase bot."
    : "The background server didn't come up in time — this is usually slow startup, not a port conflict. Quit and reopen Sandbase bot.";
  return (
...
function openDesktopViewer(owner, rawUrl, rawTitle, contextId) {
  if (!owner || owner.isDestroyed()) throw new Error("The Sandbase bot window is unavailable");
...
// Bots hand users files as markdown links to paths inside the Sandbase bot
...
// live desktop inside Sandbase bot instead of sending the person to a browser.
...
app.whenReady().then(async () => {
  ...
  if (app.isPackaged) {
    try {
      ...
      dialog.showErrorBox(
        "Sandbase bot could not start safely",
        error?.message ?? "Another process is using this Sandbase bot data folder.",
      );
      app.quit();
      return;
    }
