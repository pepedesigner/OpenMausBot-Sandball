...
test("a staged path that is gone is not used", () => {
  ...
  try {
    const present = join(workspace, "Sandbase-bot.deb");
...
test("the whole command parses into the arguments apt-get would receive", { skip: posixShell }, () => {
  const file = "/home/o'brien/.cache/openmausbot-updater/pending/Sandbase bot-0.1.44-amd64.deb";
...
test("the package marker decides the install path", () => {
  ...
  assert.equal(
    linuxPackageType({
      ...
      appImage: "/home/u/Sandbase-bot.AppImage",
